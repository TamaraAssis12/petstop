...
// Função para atualizar o array uploadedImages
function updateUploadedImagesArray() {
  const imagesContainer = document.querySelectorAll(".new-upload input[type='file']");
  const uploadedImages = Array.from(imagesContainer).map((input) => input.files[0]);
  const uploadedImagesField = document.querySelector("#uploadedImages");
